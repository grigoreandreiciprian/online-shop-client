...
const FavoriteCard: React.FC<favoriteItems> = ({ favItem }: favoriteItems) => {
  ...
  const findProduct = async () => {
    ...
    //@ts-ignore
    const item = items.filter((e: Product) => e.id === favItem.productId)[0];
  ...
  useEffect(() => {
    if (item.picture != null) {
      setPicture(`data:image/png;base64,${toBase64(item.picture.data)}`);
    }
  }, [item]);
